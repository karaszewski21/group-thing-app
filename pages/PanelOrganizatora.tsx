...
import { AVATAR_SRC } from "../data/photos";

/* ------------------------------------------------------------------ */
/*  Rodzinny grajdołek — panel organizatora (mock UI, widok mobilny)    */
/* ------------------------------------------------------------------ */
...
.org-slot.is-full{background:var(--danger-soft);color:var(--danger);}
.org-remove{
...
/* ---------- checklista potrzebnych rzeczy ---------- */
.org-check-list{
  display:flex;flex-direction:column;gap:2px;max-height:190px;overflow-y:auto;
  border:1.5px solid var(--line);border-radius:12px;padding:4px 12px;background:var(--cream);
}
.org-check-row{
  display:flex;align-items:center;gap:10px;font-size:13.5px;font-weight:600;color:var(--ink);
  padding:9px 0;
}
.org-check-row + .org-check-row{border-top:1px solid var(--line);}
.org-check-row input{width:17px;height:17px;accent-color:var(--mint);flex:none;}
.org-check-empty{font-size:12.5px;color:var(--ink-soft);padding:8px 2px;}
.org-checkbox-toggle{
  display:flex;align-items:center;gap:10px;font-size:13.5px;font-weight:700;color:var(--ink);
  border:1.5px solid var(--line);border-radius:12px;padding:12px 13px;background:var(--cream);cursor:pointer;
}
.org-checkbox-toggle input{width:18px;height:18px;accent-color:var(--mint);flex:none;}
...
const GroupsIcon = ({ c = "#1E2E27" }) => (
  <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
    <rect x="3" y="4" width="8" height="8" rx="2.4" stroke={c} strokeWidth="2.1" />
    <rect x="13" y="4" width="8" height="8" rx="2.4" stroke={c} strokeWidth="2.1" />
    <rect x="3" y="14" width="8" height="6" rx="2.2" stroke={c} strokeWidth="2.1" />
    <rect x="13" y="14" width="8" height="6" rx="2.2" stroke={c} strokeWidth="2.1" />
  </svg>
);
...
const INITIAL_GROUPS: Group[] = [
  { id: nextId(), name: "Muzyczne Maluchy", location: "Sala nr 2, Poznań Jeżyce", freeSpots: 2 },
  { id: nextId(), name: "Rytmy i grzechotki", location: "Sala nr 1, Poznań Jeżyce", freeSpots: 3 },
];

const INITIAL_TERMS: Term[] = [
  { id: nextId(), day: "27", month: "sie", groupName: "Muzyczne Maluchy", note: "16:30 · Sala nr 2 · zostały 2 miejsca", neededItems: ["Tamburyn i dzwonki", "Mata piankowa"] },
  { id: nextId(), day: "30", month: "sie", groupName: "Rytmy i grzechotki", note: "10:00 · zajęcia otwarte", neededItems: [] },
];
...
const INITIAL_ITEMS: Item[] = [
  { id: nextId(), name: "Tamburyn i dzwonki", mode: "wypożyczę" },
  { id: nextId(), name: "Książeczki dźwiękowe", mode: "zamienię" },
  { id: nextId(), name: "Mata piankowa", mode: "oddam" },
  { id: nextId(), name: "Puzzle 3+", mode: "zamienię" },
];
...
const INITIAL_GIFTS: Gift[] = [
  { id: nextId(), name: "Koc piknikowy", from: "Rodzina Lewandowskich", source: "otrzymane" },
  { id: nextId(), name: "Mata piankowa i marakasy", from: "Rodzina Mazurów", source: "pożyczone" },
];

/* ---------------- komponent ---------------- */

type View = "home" | "spotkania" | "rzeczy" | "podarki" | "profil" | "ustawienia";
type Modal = "grupa" | "termin" | "rzecz" | null;
...
export default function PanelOrganizatora() {
  ...
  const [profile, setProfile] = useState<Profile>({
    name: "Zuzanna Karaszewska",
    bio: "Prowadzę zajęcia umuzykalniające dla dzieci 0–6 lat metodą Gordona i elementami Orffa.",
  ...
  const [termForm, setTermForm] = useState<{ day: string; month: string; groupName: string; note: string; needsItems: boolean; neededItems: string[] }>({
    day: "",
    month: "",
    groupName: "",
    note: "",
    ...
    <div className="org-stage">
            ...
            <>
              ...
              <div className="org-card">
                <div className="org-card-head">
                  ...
                  <button className="org-link-btn" onClick={() => setView("spotkania")}>Zobacz wszystkie</button>
                </div>
                {terms.length === 0 && <div className="org-empty">Brak zaplanowanych terminów.</div>}
            ...
            <div className="org-card">
              ...
              <div className="org-grid">
                ...
                <div className="org-field is-full">
                  ...
                  <textarea id="org-bio" value={profile.bio} onChange={(e) => setProfile({ ...profile, bio: e.target.value })} placeholder="Kilka zdań o Tobie i sposobie prowadzenia zajęć" />
                </div>
              </div>
          ...
          {view === "ustawienia" && (
            <>
              <div className="org-card">
                ...
                <div className="org-toggle-row">
                  <div>
                    ...
                    <small>Nowe zgłoszenia i wiadomości od rodzin</small>
                  </div>
                  <button
              ...
              <div className="org-card">
                ...
                <div className="org-toggle-row">
                  <div>
                    <strong>Widoczny profil publiczny</strong>
                    <small>Rodziny mogą znaleźć Cię w wyszukiwarce</small>
                  </div>
                  <button
                    className={`org-toggle ${settings.publicProfile ? "is-on" : ""}`}
                    role="switch"
                    aria-checked={settings.publicProfile}
                    aria-label="Widoczny profil publiczny"
            ...
            <>
              ...
              <div className="org-section">
                <div className="org-section-title">
                  <div className="org-section-title-text">
                    <h2>Grupy</h2>
                    <small>{groups.length} {groups.length === 1 ? "grupa" : "grupy"}</small>
                  </div>
                  <button className="org-section-add" onClick={() => setModal("grupa")}>
                    <GroupsIcon c="#EAF2E9" /> Dodaj grupę
                  </button>
                </div>

                <div className="org-card">
                  {groups.length === 0 && <div className="org-empty">Nie masz jeszcze żadnej grupy.</div>}
                  {groups.map((g) => (
                    <div className="org-list-item" key={g.id}>
                      <span className="org-list-ico" style={{ background: "var(--mint-soft)" }}><GroupsIcon c="#12604D" /></span>
                      <div className="org-list-body">
                        <h3>{g.name}</h3>
                        <small>{g.location}</small>
                        <span className={`org-slot ${g.freeSpots ? "" : "is-full"}`}>
                          {g.freeSpots ? `wolne ${g.freeSpots} miejsca` : "lista rezerwowa"}
                        </span>
                      </div>
                      <button className="org-remove" onClick={() => removeGroup(g.id)} aria-label={`Usuń grupę ${g.name}`}>
                        <TrashIcon />
                      </button>
                    </div>
                  ))}
              ...
              <div className="org-section">
                <div className="org-section-title">
                  <div className="org-section-title-text">
                    <h2>Terminy</h2>
                    <small>{terms.length} {terms.length === 1 ? "termin" : "terminy"}</small>
                  </div>
                  <button className="org-section-add" onClick={() => setModal("termin")}>
                    <CalendarIcon c="#EAF2E9" /> Dodaj termin
                  </button>
                </div>
                ...
                <div className="org-card">
                  ...
                  {terms.map((t) => (
                    <div className="org-list-item org-date" key={t.id}>
                      ...
                      <button className="org-remove" onClick={() => removeTerm(t.id)} aria-label={`Usuń termin ${t.groupName}`}>
                        <TrashIcon />
                      </button>
                    </div>
                  ))}
      ...
      {modal === "grupa" && (
        <div className="org-modal-overlay" onClick={() => setModal(null)}>
          <div className="org-modal" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true" aria-label="Dodaj grupę">
            ...
            <div className="org-grid">
              <div className="org-field is-full">
                <label htmlFor="org-g-name">Nazwa grupy</label>
                <input id="org-g-name" value={groupForm.name} onChange={(e) => setGroupForm({ ...groupForm, name: e.target.value })} placeholder="np. Nutki dla starszaków" />
              </div>
              <div className="org-field">
                <label htmlFor="org-g-location">Lokalizacja</label>
                <input id="org-g-location" value={groupForm.location} onChange={(e) => setGroupForm({ ...groupForm, location: e.target.value })} placeholder="Sala nr 2, Poznań Jeżyce" />
              </div>
              <div className="org-field">
                <label htmlFor="org-g-free">Ile miejsc</label>
                <input id="org-g-free" type="number" min="0" value={groupForm.freeSpots} onChange={(e) => setGroupForm({ ...groupForm, freeSpots: e.target.value })} placeholder="0" />
              </div>
            </div>
            <div className="org-actions">
              <button className="org-btn org-btn-primary" onClick={addGroup}>Dodaj grupę</button>
            </div>
          </div>
        </div>
      )}
      ...
      {modal === "termin" && (
        <div className="org-modal-overlay" onClick={() => setModal(null)}>
          <div className="org-modal" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true" aria-label="Dodaj termin">
            <div className="org-modal-head">
              <h3>Dodaj termin zajęć</h3>
              <button className="org-modal-close" onClick={() => setModal(null)} aria-label="Zamknij"><CloseIcon /></button>
            </div>
            <div className="org-grid">
              <div className="org-field">
                <label htmlFor="org-t-day">Dzień</label>
                <input id="org-t-day" value={termForm.day} onChange={(e) => setTermForm({ ...termForm, day: e.target.value })} placeholder="14" />
              </div>
              <div className="org-field">
                <label htmlFor="org-t-month">Miesiąc</label>
                <input id="org-t-month" value={termForm.month} onChange={(e) => setTermForm({ ...termForm, month: e.target.value })} placeholder="wrz" />
              </div>
              <div className="org-field is-full">
                <label htmlFor="org-t-group">Grupa</label>
              ...
              <div className="org-field is-full">
                <label htmlFor="org-t-note">Godzina i uwagi</label>
                <input id="org-t-note" value={termForm.note} onChange={(e) => setTermForm({ ...termForm, note: e.target.value })} placeholder="17:00 · Park Sołacki · wstęp wolny" />
              </div>
              <div className="org-field is-full">
            ...
            <div className="org-actions">
              <button className="org-btn org-btn-primary" onClick={addTerm}>Dodaj termin</button>
            </div>
          </div>
        </div>
      )}
